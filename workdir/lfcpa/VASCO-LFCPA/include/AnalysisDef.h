...
#ifndef ANALYSISDEF_H
#define ANALYSISDEF_H
...
public:
    ...
    AnalysisDef(std::unique_ptr<llvm::Module> &, bool, bool, bool, ExecMode, bool);

    AnalysisDef(std::unique_ptr<llvm::Module> &, const string &, bool, bool, bool, ExecMode, bool);

    // AnalysisDef(bool,bool);

    // AnalysisDef(bool, const string &,bool);

    // AnalysisDef(std::unique_ptr<llvm::Module>&, bool, bool, bool);

    // AnalysisDef(std::unique_ptr<llvm::Module>&, const string &, bool, bool, bool);
    ...
    void doAnalysisFIS(int context_label, Function *func, slim::IR *optIR);
    void doAnalysisFISVec(int context_label, std::vector<Function *> vecFunc, slim::IR *optIR);
    ...
    virtual F computeDFValueFIS(llvm::Instruction &I, int depContext, F prev);
    virtual F computeDFValueFIS(BaseInstruction *I, int depContext, F prev);
    ...
    void doAnalysisVec(
        std::unique_ptr<llvm::Module> &M, slim::IR *optIR, std::vector<Function *> vecFunc, int depAnalysisContext);
    ...
    string _getFileName(llvm::Instruction *I);
    string _getFileName(BaseInstruction *I);
    unsigned _getSourceLineNumberForInstruction(llvm::Instruction *I);
    unsigned _getSourceLineNumberForInstruction(BaseInstruction *I);
};

template <class F, class B>
bool AnalysisDef<F, B>::compareIndices(std::vector<SLIMOperand *> ipVec1, std::vector<SLIMOperand *> ipVec2) const {
...
template <class F, class B>
void AnalysisDef<F, B>::setSLIMIRPointer(slim::IR *slimIRptr) {
    this->optIR = slimIRptr;
}

template <class F, class B>
BaseInstruction *AnalysisDef<F, B>::getBaseInstruction(long long id) {
...
// #########
template <class F, class B>
F AnalysisDef<F, B>::getForwardComponentAtInOfThisInstruction(BaseInstruction *I, int label) {
    // int label = getProcessingContextLabel();
    return SLIM_IN[label][I].first;
}

template <class F, class B>
F AnalysisDef<F, B>::getForwardComponentAtOutOfThisInstruction(BaseInstruction *I, int label) {
    // int label = getProcessingContextLabel();
    return SLIM_OUT[label][I].first;
}

template <class F, class B>
B AnalysisDef<F, B>::getBackwardComponentAtInOfThisInstruction(BaseInstruction *I, int label) {
    // int label = getProcessingContextLabel();
    return SLIM_IN[label][I].second;
}

template <class F, class B>
B AnalysisDef<F, B>::getBackwardComponentAtOutOfThisInstruction(BaseInstruction *I, int label) {
    // int label = getProcessingContextLabel();
    return SLIM_OUT[label][I].second;
}

template <class F, class B>
int AnalysisDef<F, B>::getCalleeContext(int callerContext, BaseInstruction *I) {
...
template <class F, class B>
bool AnalysisDef<F, B>::isGenerativeKillInitialised(int context_label, BaseInstruction *I) {
...
template <class F, class B>
pair<pair<F, B>, pair<F, B>> AnalysisDef<F, B>::getGenerativeKillAtCall(int context_label, BaseInstruction *I) {
    pair<int, BaseInstruction *> keyGenerativeKill = make_pair(context_label, I);
...
template <class F, class B>
void AnalysisDef<F, B>::setGenerativeAtCall(int context_label, BaseInstruction *I, pair<F, B> generativeAtCall) {
    pair<int, BaseInstruction *> keyGenerativeKill = make_pair(context_label, I);
...
template <class F, class B>
void AnalysisDef<F, B>::setKillAtCall(int context_label, BaseInstruction *I, pair<F, B> killAtCall) {
    pair<int, BaseInstruction *> keyGenerativeKill = make_pair(context_label, I);
...
template <class F, class B>
void AnalysisDef<F, B>::setInitialisationValueGenerativeKill(int context_label, BaseInstruction *I) {
...
//--virtual functions--
template <class F, class B>
pair<pair<F, B>, pair<F, B>> AnalysisDef<F, B>::getInitialisationValueGenerativeKill() {
    llvm::outs() << "\nThis function getInitialisationValueGenerativeKill() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
pair<F, B> AnalysisDef<F, B>::computeGenerative(int context_label, Function *Func) {
    llvm::outs() << "\nThis function computeGenerative() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
pair<F, B> AnalysisDef<F, B>::computeKill(int context_label, Function *Func) {
    llvm::outs() << "\nThis function computeKill() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
pair<F, B> AnalysisDef<F, B>::CallInflowFunction(
    int context_label, Function *Func, BasicBlock *bb, const F &a1, const B &d1, pair<F, B> generativeAtCall) {
...
template <class F, class B>
pair<F, B> AnalysisDef<F, B>::CallOnflowFunction(int context_label, BaseInstruction *I, pair<F, B> killAtCall) {
    llvm::outs() << "\nThis function CallOnflowFunction() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::getBoundaryInformationFIS() {
    llvm::outs() << "\nThis function getBoundaryInformationFIS() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::getInitialisationValueFIS() {
    llvm::outs() << "\nThis function getInitialisationValueFIS() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
bool AnalysisDef<F, B>::EqualDataFlowValuesFIS(const F &d1, const F &d2) const {
    llvm::outs() << "\nThis function EqualDataFlowValuesFIS() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::computeDFValueFIS(llvm::Instruction &I, int depContext, F prev) {
    llvm::outs() << "\nThis function computeDFValueFIS() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::computeDFValueFIS(BaseInstruction *I, int depContext, F prev) {
    llvm::outs() << "\nThis function computeDFValueFIS() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::performMeetFIS(const F &d1, const F &d2) {
    llvm::outs() << "\nThis function performMeetFIS() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::getPurelyGlobalComponentFIS(const F &dfv) const {
    llvm::outs() << "\nThis function getPurelyGlobalComponentFIS() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::getPurelyLocalComponentFIS(const F &dfv) const {
    llvm::outs() << "\nThis function getPurelyLocalComponentFIS() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::CallInflowFunctionFIS(int context_label, Function *target_function, BasicBlock *bb, const F &a1) {
    llvm::outs() << "\nThis function CallInflowFunctionFIS() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::CallOutflowFunctionFIS(
...
template <class F, class B>
F AnalysisDef<F, B>::performMergeAtCallFIS(const F &d1, const F &d2) {
    llvm::outs() << "\nThis function performMergeAtCallFIS() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
int AnalysisDef<F, B>::getDepAnalysisCalleeContext(int callerContext, BaseInstruction *inst) {
    llvm::outs() << "\nThis function getDepAnalysisCalleeContext() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
int AnalysisDef<F, B>::getDepAnalysisCalleeContext(int callerContext, llvm::Instruction &inst) {
    llvm::outs() << "\nThis function getDepAnalysisCalleeContext() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
void AnalysisDef<F, B>::doAnalysisVec(
    std::unique_ptr<llvm::Module> &Mod, slim::IR *optIR, std::vector<Function *> vecFunc, int depAnalysisContext) {
    // llvm::outs() << "\n INSIDE THE OVERLOADED FUNCTION.";

    for (std::vector<Function *>::iterator r = vecFunc.begin(); r != vecFunc.end(); r++) {
        llvm::Function *Fun = *r;
    ...
    if (modeOfExec == FSFP || modeOfExec == FSSP) {
        ...
        setCurrentAnalysisDirection(0);
        for (std::vector<Function *>::iterator r = vecFunc.begin(); r != vecFunc.end(); r++) {
            INIT_CONTEXT_FIS(*r, flow_insensitive_inflow, flow_insensitive_outflow, depAnalysisContext);
        } // end for
        if (std::is_same<B, NoAnalysisType>::value) {
            // forward analysis
            direction = "forward";
            ...
            // llvm::outs() << "\n Calling  doAnalysisFIS.............";
            doAnalysisFISVec(getProcessingContextLabel(), vecFunc, optIR);
        } // end if
    } // else
    auto stop = high_resolution_clock::now();
...
template <class F, class B>
void AnalysisDef<F, B>::doAnalysis(
    std::unique_ptr<llvm::Module> &Mod, slim::IR *optIR, Function *Func, int depAnalysisContext) {
    llvm::Module &M = *Mod;
    setCurrentModule(&M);
    ...
    */
    ...
    if (modeOfExec == FSFP || modeOfExec == FSSP) {
        ...
        if (!Func->isDeclaration()) {
            // this is an inbuilt function so doesn't need to be processed.
            F flow_insensitive_inflow;
            F flow_insensitive_outflow;
            if (std::is_same<B, NoAnalysisType>::value) {
                flow_insensitive_inflow = getBoundaryInformationFIS();
            ...
            if (std::is_same<B, NoAnalysisType>::value) {
                ...
                // while (not forward_worklist.empty()) {
                // llvm::outs() << "\n Calling  doAnalysisFIS.............";
                doAnalysisFIS(getProcessingContextLabel(), Func, optIR);
                //}
            }
        } // if not declaration
    } // end else
...
template <class F, class B>
void AnalysisDef<F, B>::INIT_CONTEXT_FIS(
...
template <class F, class B>
void AnalysisDef<F, B>::doAnalysisFISVec(int context_label, std::vector<Function *> vecFunc, slim::IR *optIR) {
    // llvm::outs() << "\n Inside doAnalysisFISVec...............";

    F dfv_for_SCC;
    F previous_dfvalue_of_this_function; // required as input to some functions previously defined
    while (not forward_worklist.empty()) // step 2
    {
        ...
        if (contains_a_method_call) {
            if (SLIM) {
                ...
                for (auto &index : optIR->func_bb_to_inst_id[{f, bb}]) {
                    ...
                    if (inst->getCall()) {
                        ...
                        // Check if the callee is in the vector of SCC
                        /// if not in SCC then proceed else do nothing
                        if (std::find(vecFunc.begin(), vecFunc.end(), target_function) == vecFunc.end()) {
                            //  llvm::outs() << "\n Target function = "<<target_function->getName()<< " contetx label =
                            ...
                            if (matching_context_label > -1) {
                                ...
                                if (debug) {
                                    llvm::outs()
                                        << "\nThe outflow value obtained for function " << target_function->getName()
                                        << " for context " << matching_context_label << " is ";
                                ...
                                setDFValueFIS(current_pair.first, target_function, outflowVal);
                                prevdfval = outflowVal;
                                if (debug) {
                                    printDataFlowValuesFIS(prevdfval);
                                    printLine(current_context_label, 1);
                                }
                            } // if matching context > -1
                        }
                    } else {
                        ...
                        // llvm::outs() << "\n Calling computeDFValueFIS 1..................";
                        F newdfval = computeDFValueFIS(inst, depAnalysisContext, prevdfval);
                        setDFValueFIS(current_pair.first, f, newdfval);

                        if (std::find(vecFunc.begin(), vecFunc.end(), f) != vecFunc.end())
                            dfv_for_SCC = performMeetFIS(dfv_for_SCC, newdfval);
            ...
            F prevdfval; // = getDFValueFIS(context_label, f);
            F dfval = NormalFlowFunctionFIS(current_pair, depAnalysisContext);
            setDFValueFIS(current_pair.first, f, dfval);
            if (std::find(vecFunc.begin(), vecFunc.end(), f) != vecFunc.end())
                dfv_for_SCC = performMeetFIS(dfv_for_SCC, dfval);
        }

        // we need the context of the processed node, so adding inside while(worklist not empty)
        // Update outflow
        // Following function is not required since the def will now have only global values
        /*   F outflowVal = getPurelyGlobalComponentFIS(getDFValueFIS(context_label,f));*/

        F outflowVal = dfv_for_SCC;
        setForwardOutflowForThisContext(current_context_label, outflowVal); // setting forward outflow
        process_mem_usage(this->vm, this->rss);
        this->total_memory = max(this->total_memory, this->vm);
    } // end inner while

    // Set the same value for all functions in SCC
    for (auto a : DFVALUE) {
        unordered_map<Function *, F> tempFIS = a.second;
        for (auto t : tempFIS) {
            // check if fun is in vecSCC
            if (std::find(vecFunc.begin(), vecFunc.end(), t.first) != vecFunc.end()) {
                // function is in SCC
                DFVALUE[a.first][t.first] = dfv_for_SCC;               // same value for all func in SCC
                setForwardOutflowForThisContext(a.first, dfv_for_SCC); // setting forward outflow
            } // end if
        } // inner for
    } // for
    ...
    /*
        ...
        if(SLIM){
            llvm::outs() << "\n==========================Displaying FIS Data flow value==========================\n";
            for (auto a : DFVALUE) {
                llvm::outs() << "\n Context : "<<a.first;
                unordered_map<Function *,F> tempFIS = a.second;
                for (auto t : tempFIS) {
                      llvm::outs() << "\n Function Name: "<<t.first->getName();
              std::pair<LFLivenessSets,long int> defblk = t.second;
                      llvm::outs() << "\n Def set: ";
                      for (auto d : defblk.first)
                          llvm::outs() <<d->getName()<<", ";
                      llvm::outs() << "\n Block set: "<<defblk.second;
                }//end inner for
            }//outer for
      }//if
    */
...
template <class F, class B>
void AnalysisDef<F, B>::doAnalysisFIS(int context_label, Function *func, slim::IR *optIR) {
    // llvm::outs() << "\n Inside doAnalysisFIS...............";
    std::map<std::pair<int, Function *>, bool> mapAlreadyProcessedFunction;
                        ...
                        if (inst->getCall()) {
                            ...
                            if (intraprocFlag) {
                                ...
                                if (matching_context_label > -1) {
                                    ...
                                    auto pos = mapAlreadyProcessedFunction.find(
                                        make_pair(current_context_label, target_function));

                                    if (pos == mapAlreadyProcessedFunction.end()) {
                                        // check if multiple calls to same callee are present then dont update DEF BLOCK
                                        // of caller. DEF BLOCK of caller should be updated only once.
                                        F a3 = getForwardOutflowForThisContext(matching_context_label);
                                        F outflow = CallOutflowFunctionFIS(
                                            current_context_label, target_function, bb, a3, prevdfval);

                                        mapAlreadyProcessedFunction[make_pair(current_context_label, target_function)] =
                                            true;
                                        F outflowVal = outflow;
                                        if (debug) {
                                        ...
                                        // At the call instruction, the value at IN should be splitted into two
                                        // components. The purely global component is given to the callee while the
                                        // mixed component is propagated to OUT of this instruction after executing
                                        // computeOUTfromIN() on it.
                                        ...
                                        /*  F a5;
                                          ...
                                          setDFValueFIS(current_pair.first, f, dfval);*/
                                        setDFValueFIS(current_pair.first, f, outflowVal);
                                        prevdfval = outflowVal;
                                        if (debug) {
                                            printDataFlowValuesFIS(prevdfval);
                                            printLine(current_context_label, 1);
                                    ...
                                    llvm::outs() << "\n Matching context not found for function: " << f->getName();
                            }
                        } else {
                            // not call instruction
                            if (debug) {
...
//  we are not able to get the dependent analysis context of the caller from the end of callee analysis)
                ...
                // TODO meet with existing value
                F prevdfval = getDFValueFIS(context_label, func);
                /// llvm::outs() << "\n #TESTING in AnalysisDef. Printing the previous value before calling
                /// normalflowfunction..........";
                /// printDataFlowValuesFIS(prevdfval);
                F dfval = NormalFlowFunctionFIS(current_pair, depAnalysisContext);
                setDFValueFIS(current_pair.first, func, dfval);
    ...
    if (debug) {
        ...
        /*
             //Displaying Data flow value obtained for all contexts
            if(SLIM){
                llvm::outs() << "\n==========================Displaying FIS Data flow
        value==========================\n"; for (auto a : DFVALUE) { llvm::outs() << "\n LABEL: "<<a.first;
                    unordered_map<Function *,F> tempFIS = a.second;
                    for (auto t : tempFIS) {
                          llvm::outs() << "\n Function Name: "<<t.first->getName();
                  std::pair<std::set<SLIMOperand*>,long int> defblk = t.second;
                          llvm::outs() << "\n Def set: ";
                          for (auto d : defblk.first)
                              llvm::outs() <<d->getName()<<", ";
                          llvm::outs() << "\n Block set: "<<defblk.second;
...
template <class F, class B>
F AnalysisDef<F, B>::NormalFlowFunctionFIS(
    ...
    if (SLIM) {
        for (auto &index : optIR->func_bb_to_inst_id[{context_object->getFunction(), &b}]) {
            ...
            dfvalue = computeDFValueFIS(
                inst, depAnalysisContext,
                getDFValueFIS(current_pair_of_context_label_and_bb.first, context_object->getFunction()));
            setDFValueFIS(current_pair_of_context_label_and_bb.first, context_object->getFunction(), dfvalue);
            /// dfvalue = performMeetFIS(getDFValueFIS(current_pair_of_context_label_and_bb.first,
            /// context_object->getFunction()),newdfvalue); setDFValueFIS(current_pair_of_context_label_and_bb.first,
            /// context_object->getFunction(), dfvalue);
...
template <class F, class B>
void AnalysisDef<F, B>::setDFValueFIS(int label, Function *func, const F &dataflowvalue) {
    DFVALUE[label][func] = dataflowvalue;
    return;
}

template <class F, class B>
F AnalysisDef<F, B>::getDFValueFIS(int label, Function *func) {
    return DFVALUE[label][func];
}

// #########
template <class F, class B>
AnalysisDef<F, B>::AnalysisDef(
...
/*template<class F, class B>
AnalysisDef<F,B>::AnalysisDef(bool debug,bool SLIM) {
    current_module = nullptr;
    context_label_counter = -1;
    this->debug = debug;
    this->direction = "";
...
template<class F, class B>
AnalysisDef<F,B>::AnalysisDef(bool debug, const string &fileName, bool SLIM) {
    current_module = nullptr;
    context_label_counter = -1;
    this->debug = debug;
    freopen(fileName.c_str(), "w", stdout);
...
template<class F, class B>
AnalysisDef<F,B>::AnalysisDef(std::unique_ptr<llvm::Module> &module, bool debug, bool SLIM, bool intraProc) {
    //bool intraProc :
    //  set true for intraprocedural analysis
    //  set false for interprocedural analysis
...
template<class F, class B>
AnalysisDef<F,B>::AnalysisDef(std::unique_ptr<llvm::Module> &module, const string &fileName, bool debug, bool SLIM, bool
intraProc) {
    //bool intraProc :
    //  set true for intraprocedural analysis
    //  set false for interprocedural analysis
...
template <class F, class B>
int AnalysisDef<F, B>::getContextLabelCounter() {
    return context_label_counter;
}

template <class F, class B>
void AnalysisDef<F, B>::setContextLabelCounter(int new_context_label_counter) {
    context_label_counter = new_context_label_counter;
}

template <class F, class B>
int AnalysisDef<F, B>::getCurrentAnalysisDirection() {
    return current_analysis_direction;
}

template <class F, class B>
void AnalysisDef<F, B>::setCurrentAnalysisDirection(int direction) {
    current_analysis_direction = direction;
}

template <class F, class B>
int AnalysisDef<F, B>::getProcessingContextLabel() const {
    return processing_context_label;
}

template <class F, class B>
void AnalysisDef<F, B>::setProcessingContextLabel(int label) {
    processing_context_label = label;
}

template <class F, class B>
unsigned int AnalysisDef<F, B>::getNumberOfContexts() {
    return ProcedureContext.size();
}

template <class F, class B>
bool AnalysisDef<F, B>::isAnIgnorableDebugInstruction(llvm::Instruction *inst) {
...
template <class F, class B>
bool AnalysisDef<F, B>::isIgnorableInstruction(BaseInstruction *inst) {
    /*if (isa<DbgDeclareInst>(inst) || isa<DbgValueInst>(inst)) {
        return true;
    }*/
    Instruction *Inst = inst->getLLVMInstruction();
...
template <class F, class B>
bool AnalysisDef<F, B>::isAnIgnorableDebugInstruction(BaseInstruction *inst) {
    return false;
}

template <class F, class B>
void AnalysisDef<F, B>::startSplitting(std::unique_ptr<llvm::Module> &mod) {
...
template <class F, class B>
void AnalysisDef<F, B>::performSplittingBB(Function &function) {
    int flag = 0;
...
template <class F, class B>
Module *AnalysisDef<F, B>::getCurrentModule() {
    return current_module;
}

template <class F, class B>
void AnalysisDef<F, B>::setCurrentModule(Module *m) {
    current_module = m;
}

template <class F, class B>
BaseInstruction *AnalysisDef<F, B>::getCurrentInstruction() const {
    return this->current_instruction;
}

template <class F, class B>
void AnalysisDef<F, B>::setCurrentInstruction(BaseInstruction *current_instruction) {
...
template <class F, class B>
void AnalysisDef<F, B>::printLine(int label, int startend) {
    /*  string Name = "";
      if (current_analysis_direction == 1) {
          Name = "FORWARDDefBlock";
      } else {
          Name = "BACKWARDDefBlock";
      }
...
template <class F, class B>
void AnalysisDef<F, B>::printModule(Module &M) {
    llvm::outs() << "--------------------------------------------" << "\n";
    for (Function &Func : M) {
...
template <class F, class B>
void AnalysisDef<F, B>::printContext() {
    llvm::outs() << "\n";
...
template <class F, class B>
void AnalysisDef<F, B>::printInOutMaps() {
    llvm::outs() << "\n";
}

template <class F, class B>
float AnalysisDef<F, B>::getTotalMemory() {
    return this->total_memory;
}

template <class F, class B>
void AnalysisDef<F, B>::printStats() {
...
template <class F, class B>
B AnalysisDef<F, B>::getFPandArgsBackward(long int Index, Instruction *I) {
    llvm::outs() << "\nThis function getFPandArgs() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::getPinStartCallee(long int index, Instruction *I, F &dfv, Function *Func) {
    llvm::outs() << "\nThis function getPinStartCallee() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
std::vector<Function *> AnalysisDef<F, B>::getIndirectCalleeFromIN(long int Index, F &dfv) {
    llvm::outs() << "\nThis function getIndirectCalleeFromIN() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
B AnalysisDef<F, B>::computeInFromOut(llvm::Instruction &I, int depContext) {
    llvm::outs() << "\nThis function computeInFromOut() has not been implemented. EXITING 1 !!\n";
    exit(-1);
}

template <class F, class B>
B AnalysisDef<F, B>::computeInFromOut(BaseInstruction *I, int depContext) {
    llvm::outs() << "\nThis function computeInFromOut() has not been implemented. EXITING 2 !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::computeOutFromIn(llvm::Instruction &I, int depContext) {
    llvm::outs() << "\nThis function computeOutFromIn() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::computeOutFromIn(BaseInstruction *I, int depContext) {
    llvm::outs() << "\nThis function computeOutFromIn() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::getBoundaryInformationForward() {
    llvm::outs() << "\nThis function getBoundaryInformationForward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
B AnalysisDef<F, B>::getBoundaryInformationBackward() {
    llvm::outs() << "\nThis function getBoundaryInformationBackward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::getInitialisationValueForward() {
    llvm::outs() << "\nThis function getInitialisationValueForward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
B AnalysisDef<F, B>::getInitialisationValueBackward() {
    llvm::outs() << "\nThis function getInitialisationValueBackward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::performMeetForward(const F &d1, const F &d2, const string pos) {
    llvm::outs() << "\nThis function performMeetForward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::performMergeAtCallForward(const F &d1, const F &d2) {
    llvm::outs() << "\nThis function performMergeAtCallForward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
B AnalysisDef<F, B>::performMeetBackward(const B &d1, const B &d2) const {
    llvm::outs() << "\nThis function performMeetBackward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
B AnalysisDef<F, B>::performMergeAtCallBackward(const B &d1, const B &d2) const {
    llvm::outs() << "\nThis function performMergeAtCallBackward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
bool AnalysisDef<F, B>::EqualDataFlowValuesForward(const F &d1, const F &d2) const {
    llvm::outs() << "\nThis function EqualDataFlowValuesForward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
bool AnalysisDef<F, B>::EqualContextValuesForward(const F &d1, const F &d2) const {
    llvm::outs() << "\nThis function EqualContextValuesForward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
bool AnalysisDef<F, B>::EqualDataFlowValuesBackward(const B &d1, const B &d2) const {
    llvm::outs() << "\nThis function EqualDataFlowValuesBackward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
bool AnalysisDef<F, B>::EqualContextValuesBackward(const B &d1, const B &d2) const {
    llvm::outs() << "\nThis function EqualContextValuesBackward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
B AnalysisDef<F, B>::getPurelyGlobalComponentBackward(const B &dfv) const {
    llvm::outs() << "\nThis function getPurelyGlobalComponentBackward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::getPurelyGlobalComponentForward(const F &dfv) const {
    llvm::outs() << "\nThis function getPurelyGlobalComponentForward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::getPurelyLocalComponentForward(const F &dfv) const {
    llvm::outs() << "\nThis function getPurelyLocalComponentForward() has not been implemented. EXITING !!\n";
    exit(-1);
}
template <class F, class B>
B AnalysisDef<F, B>::getPurelyLocalComponentBackward(const B &dfv) const {
    llvm::outs() << "\nThis function getPurelyLocalComponentBackward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
B AnalysisDef<F, B>::getMixedComponentBackward(const B &dfv) const {
    llvm::outs() << "\nThis function getMixedComponentBackward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::getMixedComponentForward(const F &dfv) const {
    llvm::outs() << "\nThis function getMixedComponentForward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
B AnalysisDef<F, B>::getCombinedValuesAtCallBackward(const B &dfv1, const B &dfv2) const {
    llvm::outs() << "\nThis function getCombinedValuesAtCallBackward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::getCombinedValuesAtCallForward(const F &dfv1, const F &dfv2) const {
    llvm::outs() << "\nThis function getCombinedValuesAtCallForward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
unsigned int AnalysisDef<F, B>::getSize(F &dfv) {
    llvm::outs() << "\nThis function getSize() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
unsigned int AnalysisDef<F, B>::getSize(B &dfv) {
    llvm::outs() << "\nThis function getSize() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
B AnalysisDef<F, B>::functionCallEffectBackward(Function *target_function, const B &dfv) {
    llvm::outs() << "\nThis function functionCallEffectBackward() has not been implemented. EXITING !!\n";
    exit(-1);
}

template <class F, class B>
F AnalysisDef<F, B>::functionCallEffectForward(Function *target_function, const F &dfv) {
...
template <class F, class B>
pair<F, B> AnalysisDef<F, B>::CallOutflowFunction(
...
template <class F, class B>
string AnalysisDef<F, B>::_getFileName(llvm::Instruction *I) {
    string sourcefile = I->getFunction()->getParent()->getSourceFileName();
    return sourcefile;
}

template <class F, class B>
string AnalysisDef<F, B>::_getFileName(BaseInstruction *I) {
    string sourcefile = I->getSourceFileName();
    return sourcefile;
}

template <class F, class B>
unsigned AnalysisDef<F, B>::_getSourceLineNumberForInstruction(llvm::Instruction *I) {
    unsigned line_num = I->getDebugLoc().getLine();
    return line_num;
}

template <class F, class B>
unsigned AnalysisDef<F, B>::_getSourceLineNumberForInstruction(BaseInstruction *I) {
...
template <class F, class B>
F AnalysisDef<F, B>::getForwardComponentAtInOfThisInstruction(llvm::Instruction &I) {
    int label = getProcessingContextLabel();
    return IN[label][&I].first;
}

template <class F, class B>
F AnalysisDef<F, B>::getForwardComponentAtInOfThisInstruction(BaseInstruction *I) {
    int label = getProcessingContextLabel();
    return SLIM_IN[label][I].first;
}

template <class F, class B>
F AnalysisDef<F, B>::getForwardComponentAtOutOfThisInstruction(llvm::Instruction &I) {
    int label = getProcessingContextLabel();
    return OUT[label][&I].first;
}

template <class F, class B>
F AnalysisDef<F, B>::getForwardComponentAtOutOfThisInstruction(BaseInstruction *I) {
    int label = getProcessingContextLabel();
    return SLIM_OUT[label][I].first;
}

template <class F, class B>
B AnalysisDef<F, B>::getBackwardComponentAtInOfThisInstruction(llvm::Instruction &I) {
    int label = getProcessingContextLabel();
    return IN[label][&I].second;
}

template <class F, class B>
B AnalysisDef<F, B>::getBackwardComponentAtInOfThisInstruction(BaseInstruction *I) {
    int label = getProcessingContextLabel();
    return SLIM_IN[label][I].second;
}

template <class F, class B>
B AnalysisDef<F, B>::getBackwardComponentAtOutOfThisInstruction(llvm::Instruction &I) {
    int label = getProcessingContextLabel();
    llvm::outs() << "label" << label;
...
template <class F, class B>
B AnalysisDef<F, B>::getBackwardComponentAtOutOfThisInstruction(BaseInstruction *I) {
    int label = getProcessingContextLabel();
    return SLIM_OUT[label][I].second;
}

template <class F, class B>
void AnalysisDef<F, B>::setForwardComponentAtInOfThisInstruction(llvm::Instruction *I, const F &in_value) {
    int label = getProcessingContextLabel();
    IN[label][I].first = in_value;
}

template <class F, class B>
void AnalysisDef<F, B>::setForwardComponentAtInOfThisInstruction(BaseInstruction *I, const F &in_value) {
    int label = getProcessingContextLabel();
    SLIM_IN[label][I].first = in_value;
}

template <class F, class B>
void AnalysisDef<F, B>::setForwardComponentAtOutOfThisInstruction(llvm::Instruction *I, const F &out_value) {
    int label = getProcessingContextLabel();
    OUT[label][I].first = out_value;
}

template <class F, class B>
void AnalysisDef<F, B>::setForwardComponentAtOutOfThisInstruction(BaseInstruction *I, const F &out_value) {
    int label = getProcessingContextLabel();
    SLIM_OUT[label][I].first = out_value;
}

template <class F, class B>
void AnalysisDef<F, B>::setBackwardComponentAtInOfThisInstruction(llvm::Instruction *I, const B &in_value) {
    int label = getProcessingContextLabel();
    IN[label][I].second = in_value;
}

template <class F, class B>
void AnalysisDef<F, B>::setBackwardComponentAtInOfThisInstruction(BaseInstruction *I, const B &in_value) {
    int label = getProcessingContextLabel();
    SLIM_IN[label][I].second = in_value;
}

template <class F, class B>
void AnalysisDef<F, B>::setBackwardComponentAtOutOfThisInstruction(llvm::Instruction *I, const B &out_value) {
    int label = getProcessingContextLabel();
    OUT[label][I].second = out_value;
}

template <class F, class B>
void AnalysisDef<F, B>::setBackwardComponentAtOutOfThisInstruction(BaseInstruction *I, const B &out_value) {
    int label = getProcessingContextLabel();
...
template <class F, class B>
pair<F, B> AnalysisDef<F, B>::getIn(int label, llvm::BasicBlock *BB) {
...
template <class F, class B>
pair<F, B> AnalysisDef<F, B>::getOut(int label, llvm::BasicBlock *BB) {
    if (SLIM) {
...
template <class F, class B>
void AnalysisDef<F, B>::setForwardIn(int label, llvm::BasicBlock *BB, const F &dataflowvalue) {
    if (SLIM) {
...
template <class F, class B>
void AnalysisDef<F, B>::setForwardOut(int label, llvm::BasicBlock *BB, const F &dataflowvalue) {
    if (SLIM) {
...
template <class F, class B>
void AnalysisDef<F, B>::setBackwardIn(int label, llvm::BasicBlock *BB, const B &dataflowvalue) {
    if (SLIM) {
...
template <class F, class B>
void AnalysisDef<F, B>::setBackwardOut(int label, llvm::BasicBlock *BB, const B &dataflowvalue) {
    if (SLIM) {
...
template <class F, class B>
F AnalysisDef<F, B>::getForwardInflowForThisContext(int context_label) {
...
template <class F, class B>
B AnalysisDef<F, B>::getBackwardInflowForThisContext(int context_label) {
...
template <class F, class B>
F AnalysisDef<F, B>::getForwardOutflowForThisContext(int context_label) {
...
template <class F, class B>
B AnalysisDef<F, B>::getBackwardOutflowForThisContext(int context_label) {
...
template <class F, class B>
void AnalysisDef<F, B>::setForwardInflowForThisContext(int context_label, const F &forward_inflow) {
...
template <class F, class B>
void AnalysisDef<F, B>::setBackwardInflowForThisContext(int context_label, const B &backward_inflow) {
...
template <class F, class B>
void AnalysisDef<F, B>::setForwardOutflowForThisContext(int context_label, const F &forward_outflow) {
...
template <class F, class B>
void AnalysisDef<F, B>::setBackwardOutflowForThisContext(int context_label, const B &backward_outflow) {
...
template <class F, class B>
Function *AnalysisDef<F, B>::getFunctionAssociatedWithThisContext(int context_label) {
...
template <class F, class B>
void AnalysisDef<F, B>::doAnalysis(std::unique_ptr<llvm::Module> &Mod, slim::IR *slimIRObj, int depAnalysisContext) {
    llvm::Module &M = *Mod;
    setCurrentModule(&M);
...
template <class F, class B>
void AnalysisDef<F, B>::INIT_CONTEXT(
...
template <class F, class B>
int AnalysisDef<F, B>::check_if_context_already_exists(
...
template <class F, class B>
void AnalysisDef<F, B>::doAnalysisForward() {
    while (not forward_worklist.empty()) // step 2
    {
        // step 3 and 4
        pair<pair<int, BasicBlock *>, int> worklistElement = forward_worklist.workDelete();
...
template <class F, class B>
F AnalysisDef<F, B>::NormalFlowFunctionForward(
    pair<int, BasicBlock *> current_pair_of_context_label_and_bb, int depAnalysisContext) {
    BasicBlock &b = *(current_pair_of_context_label_and_bb.second);
...
template <class F, class B>
void AnalysisDef<F, B>::doAnalysisBackward() {
...
template <class F, class B>
B AnalysisDef<F, B>::NormalFlowFunctionBackward(
    pair<int, BasicBlock *> current_pair_of_context_label_and_bb, int depAnalysisContext) {
    BasicBlock &b = *(current_pair_of_context_label_and_bb.second);
...
template <class F, class B>
pair<F, B> getAnalysisInAtInstruction(BaseInstruction *I, int context_label, AnalysisDef<F, B> *analysisObj) {
...
template <class F, class B>
pair<F, B> getAnalysisOutAtInstruction(BaseInstruction *I, int context_label, AnalysisDef<F, B> *analysisObj) {
