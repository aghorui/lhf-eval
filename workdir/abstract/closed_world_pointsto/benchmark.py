...
def generate_normal_intptr_list(
	...
	while count < size:
		n = (generate_normal_int(mean, stddev // 8, min_value, max_value),
...
def generate_operation_str(optype: Operation, arg1: int, arg2: int, result: Set[Tuple[int, int]]) -> str:
	return \
		optype.value     + "\n" + \
		str(arg1) + "\n" + \
		str(arg2) + "\n" + \
...
def generate_corpus_str(corpus: List[Set[int]]) -> str:
	...
	ret += f"{len(corpus)}\n"
	for i in corpus:
		ret += f"{pointsto_container_to_str(i)}\n"
